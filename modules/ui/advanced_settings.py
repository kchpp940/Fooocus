...
from modules.ui.generation import (
    format_preset_details_html, build_preset_data_from_ui, inpaint_mode_change,
    random_checked
)
...
def create_settings_tab(gradio_root, output_format_ref):
    ...
    with gr.Accordion(label='Aspect Ratios', open=False, elem_id='aspect_ratios_accordion') as aspect_ratios_accordion:
        components['aspect_ratios_accordion'] = aspect_ratios_accordion
        ...
        aspect_ratios_selection.change(
            lambda x: None, inputs=aspect_ratios_selection,
    ...
    image_number = gr.Slider(
        label='Image Number', minimum=1, maximum=modules.config.default_max_image_number,
    ...
    negative_prompt = gr.Textbox(
        label='Negative Prompt', show_label=True, placeholder="Type prompt here.",
    ...
    components['update_history_link_fn'] = update_history_link
...
def create_models_tab(gradio_root):
    ...
    with gr.Group():
        with gr.Row():
            base_model = gr.Dropdown(
                label='Base Model (SDXL only)', choices=modules.config.model_filenames,
                value=modules.config.default_base_model_name, show_label=True
            )
            components['base_model'] = base_model

            refiner_model = gr.Dropdown(
                label='Refiner (SDXL or SD 1.5)', choices=['None'] + modules.config.model_filenames,
        ...
        refiner_switch = gr.Slider(
            label='Refiner Switch At', minimum=0.1, maximum=1.0, step=0.0001,
        ...
        for i, (enabled, filename, weight) in enumerate(modules.config.default_loras):
            with gr.Row():
                ...
                lora_weight = gr.Slider(
                    label='Weight', minimum=modules.config.default_loras_min_weight,
    ...
    with gr.Row():
        refresh_files = gr.Button(
            label='Refresh', value='\U0001f504 Refresh All Files',
...
def create_advanced_tab(gradio_root):
    ...
    sharpness = gr.Slider(
        label='Image Sharpness', minimum=0.0, maximum=30.0, step=0.001,
    ...
    with gr.Column(visible=modules.config.default_developer_debug_mode_checkbox) as dev_tools:
        ...
        with gr.Tab(label='Debug Tools'):
            ...
            adm_scaler_end = gr.Slider(
                label='ADM Guidance End At Step', minimum=0.0, maximum=1.0,
            ...
            refiner_swap_method = gr.Dropdown(
                label='Refiner swap method', value=flags.refiner_swap_method,
            ...
            vae_name = gr.Dropdown(
                label='VAE', choices=[modules.flags.default_vae] + modules.config.vae_filenames,
            ...
            disable_preview = gr.Checkbox(
                label='Disable Preview', value=modules.config.default_black_out_nsfw,
            ...
            black_out_nsfw = gr.Checkbox(
                label='Black Out NSFW', value=modules.config.default_black_out_nsfw,
                interactive=not modules.config.default_black_out_nsfw,
            ...
            if not args_manager.args.disable_metadata:
                ...
                save_metadata_to_images.change(
                    lambda x: gr.update(visible=x), inputs=[save_metadata_to_images],
    ...
    components['inpaint_ctrls'] = inpaint_ctrls
...
def bind_advanced_column_events(
    components, output_format_ref,
