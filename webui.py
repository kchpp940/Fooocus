        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            ...
            with gr.Tab(label='Advanced'):
                ...
                def refresh_files_clicked():
                    ...
                    resource_service.refresh_all_files()
                    model_files = resource_service.get_filenames(ResourceCategory.CHECKPOINT)
                    lora_files = resource_service.get_filenames(ResourceCategory.LORA)
                    vae_files = resource_service.get_filenames(ResourceCategory.VAE)
                    results = [gr.update(choices=model_files)]
                    results += [gr.update(choices=['None'] + model_files)]
                    results += [gr.update(choices=[flags.default_vae] + vae_files)]
                    if not args_manager.args.disable_preset_selection:
                        results += [gr.update(choices=modules.config.available_presets)]
                    for i in range(modules.config.default_max_lora_number):
                        results += [gr.update(interactive=True),
                                    gr.update(choices=['None'] + lora_files), gr.update()]
                    return results
